...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMdtestHdle;                    /**< MDTEST handle */
  uint16_t  CustomWriteHdle;                  /**< Write handle */
  uint16_t  CustomNotifyHdle;                  /**< Notify handle */
...
#define COPY_MDTEST_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_WRITE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_NOTIFY_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x02,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomNotifyHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_NOTIFY_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_NOTIFY_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE :
          ...
          if (read_req->Attribute_Handle == (CustomContext.CustomNotifyHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for MDTEST +
   *                                2 for Write +
   *                                2 for Notify +
   *                                1 for Notify configuration descriptor +
   *                              = 6
   *
  ...
  COPY_MDTEST_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMdtestHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: MDTEST, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: MDTEST , handle = 0x%04x \n\r", CustomContext.CustomMdtestHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomMdtestHdle,
                          UUID_TYPE_128, &uuid,
                          SizeWrite,
                          CHAR_PROP_WRITE,
  ...
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : WRITE , handle = 0x%04x \n\r", CustomContext.CustomWriteHdle);
  }
   ...
   *  Notify
   */
  COPY_NOTIFY_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMdtestHdle,
                          UUID_TYPE_128, &uuid,
                          SizeNotify,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : NOTIFY, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : NOTIFY , handle = 0x%04x \n\r", CustomContext.CustomNotifyHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_WRITE:
      ret = aci_gatt_update_char_value(CustomContext.CustomMdtestHdle,
                                       CustomContext.CustomWriteHdle,
                                       0, /* charValOffset */
    ...
    case CUSTOM_STM_NOTIFY:
      ret = aci_gatt_update_char_value(CustomContext.CustomMdtestHdle,
                                       CustomContext.CustomNotifyHdle,
                                       0, /* charValOffset */
                                       SizeNotify, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value NOTIFY command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value NOTIFY command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_WRITE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMdtestHdle, CustomContext.CustomWriteHdle, SizeWrite, pPayload);
    ...
    case CUSTOM_STM_NOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMdtestHdle, CustomContext.CustomNotifyHdle, SizeNotify, pPayload);
