...
bool ParseConstants(uint8_t* data);
void GetProbe(char* ucConst,long double* db, int i,unsigned char* myStr);
...
//        flash_jump_to_app();
        FlashWriteWearableConstants();
        getSavedConfigurationValues();
        (void)uart_transmit_str((uint8_t*)"\n\n\r   Wearable ID & Size set and probe coefficients successfully updated");
        //FlashSaveDeviceInfo(&sDeviceInfo);
        HAL_Delay(2000);
        uart_transmit_str((uint8_t*)"\n\n\r   Please restart the device !");
        UpdateRestartDevice();
        while(1);
        break;
      /* Abort from host. */
...
static xmodem_status xmodem_handle_packet(uint8_t header)
{
  ...
  ParseConstants(received_packet_data);
#if 0
...
void GetProbe(char* ucConst,long double* db, int i,unsigned char* myStr)
{
    char *pStart = NULL;
	char sign;
	long double mulFactor = 1;
	unsigned char coeff=0;
    pStart = strstr((char*)myStr,ucConst);
    pStart += 2;
	for(; *pStart != ']'; pStart++)
	{
		coeff = (coeff*10)+(*pStart - 48);
	}
	if(coeff == i)
	{
		pStart += 2;
		sign = *pStart;
		pStart += 3;
		db[coeff] = 0; //modified... i missed this line.
		for(; *pStart != ';'; pStart++)
		{
			db[coeff] = (db[coeff]*10)+(*pStart - 48);
			mulFactor *= 10;
		}
		if(sign == '-')
			db[coeff] *= -1;
